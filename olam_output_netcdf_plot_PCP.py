...
def interpolate_3grids(olam_out, latlons, outgrids, var_list, variables, mask=None):
    for index in var_list:
        ...
        values = values[1:, :]
        interp_data = [
            np.squeeze(griddata(latlons, values[:, levels], outgrids, method="nearest"))
            for levels in range(values.shape[1])
        ]
        variables[index] = (["level", "lat", "lon"], np.array(interp_data))
        if mask is not None:
            variables[index] = (
                ["level", "lat", "lon"],
                np.array(interp_data) * mask * 1,
            )
    return variables
...
gfile = (
    "/rstor/akumar/USA/Hurricane_IDA/OLAM/olam-4.12.2/build_test/sfcfiles/gridfile2.h5"
)
...
lfile = "landh2.h5"
sfile = "seah2.h5"
olamout = "Bahamas-H-2019-10-05-000000.h5"
...
glat = gridfile["GLATM"]
topo = gridfile["TOPM"]
topo_data = np.squeeze(
    griddata(np.stack([glon, glat]).T, topo, outgrids, method="cubic")
)
...
id = np.array(
    [
        [np.where(np.array(lists) == vars)[0] for vars in input_variables]
        for lists in np.array([var_list1, var_list2, var_list3, var_list4])
    ]
)
...
for var_list_id in range(len(input_variables)):
    indices.append(
        [np.where(np.array([len(var) for var in id[:, var_list_id]]) == True)][0][0]
    )
...
print(
    f"The following variable(s) are not available in the input data: {np.array(input_variables)[remove_id]}"
)
...
group1 = list(
    np.array(input_variables)[np.array([var.shape[0] for var in id[0, :]]) == 1]
)
group2 = list(
    np.array(input_variables)[np.array([var.shape[0] for var in id[1, :]]) == 1]
)
group3 = list(
    np.array(input_variables)[np.array([var.shape[0] for var in id[2, :]]) == 1]
)
group4 = list(
    np.array(input_variables)[np.array([var.shape[0] for var in id[3, :]]) == 1]
)
...
if bool(group1):
    ...
    latlons = latlons[1:, :]
    variables = interpolate_grids(
        olam_out, latlons, outgrids, group1, variables, mask=None
    )
...
if bool(group2):
    ...
    latlons = latlons[1:, :]
    variables = interpolate_grids(
        olam_out, latlons, outgrids, group2, variables, mask=land_mask.mask
    )
...
if bool(group3):
    ...
    latlons = latlons[1:, :]
    variables = interpolate_grids(
        olam_out, latlons, outgrids, group3, variables, mask=ocean_mask.mask
    )

if bool(group4):
    latlons = np.vstack((gridfile["GLONW"], gridfile["GLATW"])).T
    latlons = latlons[1:, :]
    variables = interpolate_3grids(
        olam_out, latlons, outgrids, group4, variables, mask=None
    )
...
for metadata_id in meta_data:
    coordinates_metadata[metadata_id] = (
        ["level"],
        np.squeeze(gridfile[metadata_id].values),
    )
...
netcdf_file = X.Dataset(variables, coords=coordinates)
...
for index in list(netcdf_file.keys()):
    ...
    try:
        ...
        netcdf_file[index].attrs = {
            "full_name": var_meta["full_name"].values[id],
            "units": var_meta["units"].values[id],
            "min": np.round(np.nanmin(netcdf_file[index].values), 3),
            "max": np.round(np.nanmax(netcdf_file[index].values), 3),
        }
    except:
        netcdf_file[index].attrs = {
            "min": np.round(np.nanmin(netcdf_file[index].values), 3),
...
netcdf_file.to_netcdf("test1.nc")
